<script>
	...
	onMount(async () => {
		...
		search = new Search({
			...
			processTerm: (term) => (stop_words.has(term) ? null : term.toLowerCase())
		});
		...
		<ul class="list-none px-0 h-[300px] fancy-scrollbar mt-2 overflow-y-auto">
			{#each results as item}
				<li
					...
					class="border-b dark:border-gray-700 py-2 text-base hoverable"
				>
					<a
						href="/blog/{item.slug}"
						class="block text-inherit hover:text-inherit visited:text-inherit"
					>
